...
return (
	...
	{bookingList.length>0?bookingList.map((item,index)=>(
		...
		<Image src={item.documentId.business.data.documentId?.image?.data?.documentId?.url}
		...
		<h2 className='font-bold text-[18px] items-center flex justify-between'>{item.documentId.business.data.documentId.Name}
		{!expired&&<CancelBooking onContinueClick={()=>onDeleteBooking(item)}/>}
		</h2>
		<h2 className='flex gap-2 text-gray-500'> <MapPin className='text-primary h-5 w-5'/>
		{item.documentId.business.data.documentId.Address}</h2>
		<h2 className='flex gap-2'><Calendar className='text-primary h-5 w-5'/> Appointment On:
		{ moment(item.documentId.Date).format('DD-MMM-YYYY')} </h2>
		<h2 className='flex gap-2'><Clock className='text-primary h-5 w-5'/> At Time : {item.documentId.Time} </h2>
		</div>
	  </div>
